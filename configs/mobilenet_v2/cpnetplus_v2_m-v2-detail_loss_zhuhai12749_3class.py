...
#        drop_out_ratio=0.1,
        ...
        norm_cfg=norm_cfg,
        align_corners=False,
        c0_in_channels=24,  #detail——loss那层的通道数需要和detail_channels一致，如果需要和最终拼接就输入24，如果不拼接就输出-1
        ...
        detail_index=1,#detail_loss对应的层数
        detail_channels=24,
        arm_channels=-1,#不使用输入小于0的值
        loss_decode=dict(
            type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.0, alpha=1, gamma=0,use_pixel_weight=True,pa=2,only_block=True,sky=True,sky_a=1),
        loss_prior_decode=dict(type='AffinityLoss', loss_weight=1.0),
        loss_detail_loss=dict(type='DetailAggregateLoss', loss_weight=1.0, use_x8=True,only_x1=False),
    ),
    auxiliary_head=dict(
        type='FCNHead',
        in_channels=64,
