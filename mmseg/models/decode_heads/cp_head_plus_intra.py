...
class CPHeadPlusIntra(BaseDecodeHead):
    ...
    def __init__(self,
                 ...
                 c1_channels=0,
                 **kwargs):
        super(CPHeadPlusIntra, self).__init__(**kwargs)
        ...
        self.bottleneck = ConvModule(
            self.in_channels + self.prior_channels + c1_channels,
            self.channels,
            3,
            padding=1,
            conv_cfg=self.conv_cfg,
    ...
    def forward(self, inputs): #inputs 基于in_index选择对应层的特征，通过transfrom_input选出来，inputs本身会把4层都传过来
        ...
        cp_outs = torch.cat([x, intra_context], dim=1)
    ...
    def losses(self, seg_logit, seg_label):
        ...
        loss = dict()
        loss.update(super(CPHeadPlusIntra, self).losses(seg_logit, seg_label))
