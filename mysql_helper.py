...
@app.route('/init_insert/statistic_two_class/<modelname>/<roc>/<dataset>', methods=['post','GET'])
def init_insert_statistic_two_class(modelname,roc,dataset):
    sql_cmd="insert into statistic_two_class(model_name,roc_threshold,create_date,metric_status,dataset) values('" + modelname + "','" + str(roc) + "','" +datetime.datetime.now().strftime("%Y-%m-%d") + "','I','" + dataset +"');"
...
def query_statistic_status(modelname,roc,dataset):
    sql_cmd = "select metric_status from statistic_two_class where model_name = '" + modelname + "' and roc_threshold = '" + str(roc) + "' and dataset = '" + dataset + "';"
    return jsonify(mysql_query(sql_cmd))
...
def update_statistic_status(modelname,roc,dataset):
    sql_cmd = "update statistic_two_class set metric_status='Y' where model_name = '" + modelname + "' and roc_threshold = '" + str(roc) + "' and dataset = '" + dataset + "';"
    return jsonify(mysql_query(sql_cmd))
...
def update_statistic():
    ...
    sql_cmd =   "update statistic_two_class set "    \
                "aAcc='" + str(aAcc) + "', " \
                "mioU='" + str(mioU) + "', " \
                "macc='" + str(macc) + "', " \
                "mFscore='" + str(mFscore) + "', " \
